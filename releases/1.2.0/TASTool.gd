...
const GOOBPLAYABILITY_VERSION: = "1.2.0"
...
func _show_responsive_popup(title: String, message: String, accept_text := "OK", accept_method := "_close_tool_popup", cancel_text := "") -> void:
	...
	panel.rect_min_size = Vector2(clamp(viewport_size.x * 0.64, 520.0, 920.0), clamp(viewport_size.y * 0.72, 390.0, 720.0))
	...
	body.rect_min_size = Vector2(max(420.0, panel.rect_min_size.x - 76.0), max(260.0, float(estimated_lines) * 31.0))
...
# The native level terrain is a two-stage presentation pipeline. A
# GDViewportSDF copies the native camera transform into an SDF viewport, then
# FullScreenLevelRect updates the themed fullscreen terrain rectangle/shader
# from that viewport. Both scripts normally update in _process(), so pausing
# the gameplay tree without exempting them freezes the themed terrain to the
# camera rectangle that was visible when the Timeline Editor opened. Freecam
# then reveals only raw level primitives outside that old rectangle.
...
func _refresh_macro_editor_presentation() -> void:
	# Preserve pipeline order even if tree order differs: first update the SDF
	# viewport transform, then size/reposition the fullscreen themed rect.
